...
if (empty($kodeDipilih)) {
    $_SESSION['error'] = "Silakan jawab minimal satu pertanyaan dengan 'Ya'.";
    header('Location: survei.php');
    exit;
}

// 2. Dari kode gejala, cari id_gejala numeriknya
$inList = "'" . implode("','", array_map('mysqli_real_escape_string', array_fill(0, count($kodeDipilih), $conn), $kodeDipilih)) . "'"; 
$sql = "SELECT id_gejala, kode_gejala FROM gejala WHERE kode_gejala IN ($inList)";
...
if (empty($gejalaTerpilih)) {
    $_SESSION['error'] = "Tidak ada gejala yang valid ditemukan.";
    header('Location: survei.php');
    exit;
}
...
while ($k = mysqli_fetch_assoc($kondisi)) {
    ...
    foreach ($gejalaTerpilih as $id_gejala) {
        $q = mysqli_query($conn,
            "SELECT value_cf 
             FROM know_base 
             WHERE id_kondisi = " . intval($id_kondisi) . " 
               AND id_gejala = " . intval($id_gejala)
        );
        
        if (!$q) {
            continue; // Skip on database error for this specific query
        }
...
if (empty($cf_kondisi)) {
    $_SESSION['error'] = "Tidak dapat menentukan tipe tubuh berdasarkan jawaban yang diberikan.";
    header('Location: survei.php');
    exit;
}
